...
class GraphQaModel(LightningModule):
    def __init__(self, hparams):
        ...
        if self.hparams.embedding_op == "concat":
            ...
            self.question_expert = ExpertModel(num_layers=self.hparams.n_expert_layers, input_size=self.roberta.config.hidden_size,
                                               output_size=self.roberta.config.hidden_size,
                                               hidden_size=self.roberta.config.hidden_size)
            self.graph_expert = GraphExpert(num_layers=self.hparams.n_expert_layers, input_size=self.roberta.config.hidden_size,
                                            output_size=self.roberta.config.hidden_size, hidden_size=self.roberta.config.hidden_size)
            self.graph_expert_2 = ExpertModel(num_layers=self.hparams.n_expert_layers, input_size=self.roberta.config.hidden_size,
                                              output_size=self.roberta.config.hidden_size,
                                              hidden_size=self.roberta.config.hidden_size)
        ...
        self.rgcn = InfluenceGraphGNN(num_in_features=self.hparams.h_dim, num_out_features=self.hparams.h_dim,
                                      num_relations=self.hparams.num_relations, rgcn=self.hparams.use_rgcn)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--n_expert_layers", default=1, type=int,
                            help="Number of layers for MOE.")
        ...
        parser.add_argument("--embedding_op",  type=str,
                            help="The operation to be used to concatenate the embeddings")
        parser.add_argument("--non_pretrained_lr_factor",  type=float, default=10.0,
                            help="The factor by which the LR of non-pretrained modules is multiplied")
        parser.add_argument(
    ...
    def set_train_steps(hparams, train_dataloader):
        ...
        train_steps = (hparams.max_epochs *
                       train_batches) // hparams.accumulate_grad_batches
        hparams.train_steps = train_steps
        hparams.warmup_steps = int(hparams.train_steps * hparams.warmup_prop)

    def configure_optimizers(self):

        def _is_roberta(param_name: str): return "roberta" in param_name

        params = list(self.named_parameters())
        grouped_parameters = [
            {"params": [p for n, p in params if _is_roberta(
                n)], 'lr': self.hparams.lr},
            {"params": [p for n, p in params if not _is_roberta(
                n)], 'lr': self.hparams.lr * self.hparams.non_pretrained_lr_factor},
        ]
    ...
    def forward(self, batch):
        ...
        nodes_cls_embedding = self.encode_nodes(
            graphs, graph_masks)  # (B, num_nodes, h_dim)
        ...
        if self.hparams.embedding_op == "concat":
            ...
            graph_expert_output, graph_expert_probs = self.graph_expert(
                nodes_cls_embedding)

            #  the gating is determined by the original input
            input = torch.cat(
                [question_cls_embeddeding, graph_expert_output], dim=-1)  # B x 1 x 1536
            ...
            expert_probs = self.softmax(
                experts_logits).unsqueeze(1)  # B x 1 x 2

            # Run the second level of experts
            question_expert_output = self.question_expert(
                question_cls_embeddeding)
            graph_expert_output_l2 = self.graph_expert_2(graph_expert_output)
            output_level_2 = torch.cat(
                [question_expert_output, graph_expert_output_l2], dim=-1)  # B x 1 x 1536

            # Combine them using the gates
            combined_output = expert_probs.mul(
                output_level_2.view(bsz, hsz, 2))
    ...
    def setup(self, stage):
        if stage == 'fit':
            ...
            self.train_steps = (
                self.hparams.max_epochs * train_batches) // self.hparams.accumulate_grad_batches

    def forward_roberta(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, token_type_ids: torch.Tensor = None):
        """Returns the pooled token from BERT
        """
        last_hidden_state = self.roberta(
            input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask).last_hidden_state
    ...
    def encode_nodes(self, graphs, graph_masks):
        ...
        # Step 1: encode all the nodes using Roberta
        batch_sz = len(graphs) // InfluenceGraphNNData.num_nodes_per_graph
        cls_embedding = self.forward_roberta(
            input_ids=graphs, attention_mask=graph_masks)
        cls_embedding = cls_embedding.view(
            batch_sz, InfluenceGraphNNData.num_nodes_per_graph, -1)  # (num_graph, num_nodes, 768)

        # Step 2: make torch geometric graphs
        pyg_graphs = []
        for i in range(batch_sz):
            pyg_graphs.append(Data(x=cls_embedding[i],
                                   edge_index=torch.tensor(
                                       InfluenceGraphNNData.edge_index).long(),
                                   edge_type=torch.tensor(
                                       InfluenceGraphNNData.edge_type).long(),
                                   num_nodes=len(InfluenceGraphNNData.node_index)))

        # Step 3: Run GCN
        graphs_batched = Batch.from_data_list(
            pyg_graphs)  # pytorch geometric batch
        x = self.rgcn(graphs_batched)  # (bsz x num_nodes, 768)
        x = x.view(batch_sz, -1, self.hparams.h_dim)  # (bsz, num_nodes, 768)
        return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_gates', expert_probs.squeeze(1)[:, 1].mean(), on_step=True,
                 on_epoch=True, prog_bar=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_gates', expert_probs.squeeze(1)[:, 1].mean(), on_step=True,
                 on_epoch=True, prog_bar=True, sync_dist=True)
        return {"loss": loss, "attention": attention}
    ...
    def validation_epoch_end(self, outputs: List[Any]) -> None:
        total_entropy, num_elements = 0., 0
        tmp = []
        ...
        with torch.no_grad():
            ...
            self.logger.experiment.add_histogram("val_nodes_attended_to", torch.cat(
                tmp, 0), self.current_epoch)  # histogram over the nodes attended to
            self.logger.experiment.add_histogram("val_nodes_attended_to_top2", torch.tensor(
                top2_attn), self.current_epoch)  # histogram over the nodes attended to

            self.log('val_attention_entropy', total_entropy / num_elements,
                     on_epoch=True, prog_bar=True)  # hopefully goes down as attention becomes more peaky
