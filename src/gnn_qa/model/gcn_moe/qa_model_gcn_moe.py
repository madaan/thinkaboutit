...
class GraphQaModel(LightningModule):
    def __init__(self, hparams):
        ...
        if self.hparams.embedding_op == "concat":
            ...
            self.question_expert = ExpertModel(
                num_layers=self.hparams.n_expert_layers,
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--n_expert_layers", default=1, type=int, help="Number of layers for MOE.")
    ...
    def configure_optimizers(self):
        ...
        grouped_parameters = [
            ...
            {
                "params": [p for n, p in params if not _is_roberta(n)],
                "lr": self.hparams.lr * self.hparams.non_pretrained_lr_factor,
            },
        ]
    ...
    def encode_nodes(self, graphs, graph_masks):
        ...
        for i in range(batch_sz):
            pyg_graphs.append(
                Data(
                    x=cls_embedding[i],
                    edge_index=torch.tensor(InfluenceGraphNNData.edge_index).long(),
                    edge_type=torch.tensor(InfluenceGraphNNData.edge_type).long(),
                    num_nodes=len(InfluenceGraphNNData.node_index),
                )
            )

        # Step 3: Run GCN
        graphs_batched = Batch.from_data_list(pyg_graphs)  # pytorch geometric batch
