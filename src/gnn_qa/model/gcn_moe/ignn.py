...
class InfluenceGraphGNN(LightningModule):
    def __init__(self, num_in_features: int, num_out_features: int, num_relations: int, rgcn: bool = True):
        super(InfluenceGraphGNN, self).__init__()
        self.rgcn = rgcn
        if rgcn:
            logging.info(f"Initializing RGCN with {num_relations} edge types.")
    ...
    def forward_rgcn(self, data):
        ...
        x = F.relu(self.conv1(x, edge_index=edge_index, edge_type=edge_type))

        x = F.dropout(x, training=self.training)

        x = F.relu(self.conv2(x, edge_index=edge_index, edge_type=edge_type))
