...
# class InfluenceGraphNNData:
#     #  Each node in this graph is constructed by merging the neighboring nodes and includes the relation.

#     node_index = [('VX', 0), ('ZX', 1), ('XW', 2), ('XY', 3),
#                   ('UY', 4)]  # , ('WL', 5), ('WM', 6), ('YL', 7),  ('YM', 8)]
#     num_nodes_per_graph = len(node_index)
#     # Two pair-nodes have an edge if they have a common var (e.g, VX - ZX)
#     edge_index = [[0, 0, 0, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 5, 5, 6, 7],
#                   [1, 2, 3, 2, 3, 3, 5, 6, 4, 7, 8, 7, 8, 6, 7, 8, 8]]

#     EDGE_TYPE_HELPS, EDGE_TYPE_HURTS = 0, 1
#     edge_features = [InfluenceGraphNNData.EDGE_TYPE_HURTS,
...
#         igraph = InfluenceGraph(graph_dict)
#         relation = InfluenceGraphNNData.get_pairwise_relations(igraph)

#         igraph.nodes_dict["L"] = igraph.nodes_dict["dec"]
#         igraph.nodes_dict["M"] = igraph.nodes_dict["acc"]

#         node_sentences = []
#         for i in range(len(InfluenceGraphNNData.node_index)):
#             src, tgt = InfluenceGraphNNData.node_index[i][0]
#             rel = relation[(src, tgt)]

#             src_node_text = node_func(src)
#             tgt_node_text = node_func(tgt)
#             st = src + tgt
#             # if st in {'WL', 'WM', 'YL', 'YM'}:
#             #     node_sentences.append(f"{src_node_text} {tgt_node_text}")
#             # else:
#             #     node_sentences.append(f"{src_node_text} <{rel}> {tgt_node_text}")
#             node_sentences.append(f"{src_node_text} <{rel}> {tgt_node_text}")
#             # print(f"{src} {tgt} {src_node_text} <{rel}> {tgt_node_text}")
...
#     @staticmethod
#     def get_pairwise_relations(igraph):
#         """Partly depends on the igraph because of "Y affects more/less"
#         """
#         pair_nodes = {}
#         nodes = ["V", "Z", "X", "U", "W", "Y"]
#         for src in nodes:
#             for (tgt, reln) in igraph.edges[src]:
#                 if tgt == "dec":
#                     tgt = "L"
#                 if tgt == "acc":
#                     tgt = "M"
#                 pair_nodes[(src, tgt)] = reln
#         return pair_nodes
