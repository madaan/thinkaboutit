...
def load_model(ckpt, graphs_file_name):
    ...
    dm = GraphQaDataModule(basedir=model.hparams.dataset_basedir, tokenizer_name=model.hparams.model_name,
                           batch_size=model.hparams.batch_size,  graphs_file_name=graphs_file_name, node_op=model.hparams.node_op)
...
def eval(model, dataloader, influence_graphs, extract_paths: bool = False, paths_output_loc: str = None):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=len(dataloader)):
            ...
            batch_graph_expert_probs, logits, acc, batch_augmentation_expert_probs = model([data.cuda() for data in batch])
            ...
            total_correct += (acc.item() * batch_size)
            print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--graphs_file_name", help="File that contains the generated influence graphs.", type=str)
