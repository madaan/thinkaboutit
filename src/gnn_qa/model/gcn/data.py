...
class GraphQaDataModule(pl.LightningDataModule):
    def __init__(self, basedir: str, tokenizer_name: str, batch_size: int,
                 graphs_file_name: str, num_workers: int = 16):
        super().__init__()
...
class InfluenceGraphNNData:
    ...
    def make_data_from_dict(graph_dict: dict, tokenizer, max_length=30):
        ...
        encoding_dict = tokenizer(
            node_sentences, max_length=max_length, truncation=True)
        return Data(graph_id=str(igraph.graph_id), num_nodes=len(InfluenceGraphNNData.node_index), tokens=encoding_dict["input_ids"],
                    edge_index=torch.tensor(InfluenceGraphNNData.edge_index).long(), edge_attr=torch.tensor(InfluenceGraphNNData.edge_type).long())
