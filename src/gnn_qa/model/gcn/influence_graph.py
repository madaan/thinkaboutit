...
class InfluenceGraph:
    ...
    nodes = ["V", "Z", "X", "U", "W", "Y", "para_outcome_accelerate", "para_outcome_decelerate"]
    ...
    def __str__(self) -> str:
        hurts = ' - hurts -'
        helps = ' - helps -'
        v = f"V : {random.choice(self.nodes_dict['V'])}" if len(
            self.nodes_dict['V']) > 0 else ''
        z = f"Z : {random.choice(self.nodes_dict['Z'])}" if len(
            self.nodes_dict['Z']) > 0 else ''
        x = f"X : {random.choice(self.nodes_dict['X'])}" if len(
            self.nodes_dict['X']) > 0 else ''
        u = f"U : {random.choice(self.nodes_dict['U'])}" if len(
            self.nodes_dict['U']) > 0 else ''
        w = f"W : {random.choice(self.nodes_dict['W'])}" if len(
            self.nodes_dict['W']) > 0 else ''
        y = f"Y : {random.choice(self.nodes_dict['Y'])}" if len(
            self.nodes_dict['Y']) > 0 else ''
        l = f"L : {random.choice(self.nodes_dict['dec'])}" if len(
            self.nodes_dict['dec']) > 0 else ''
        m = f"M : {random.choice(self.nodes_dict['acc'])}" if len(
    ...
    def to_dot(self) -> str:
        ...
        helps = '[label=helps]'
        arrow = '->'
        v = _wrap_quotes(f"V : {random.choice(self.nodes_dict['V'])}" if len(
            self.nodes_dict['V']) > 0 else '')
        z = _wrap_quotes(f"Z : {random.choice(self.nodes_dict['Z'])}" if len(
            self.nodes_dict['Z']) > 0 else '')
        x = _wrap_quotes(f"X : {random.choice(self.nodes_dict['X'])}" if len(
            self.nodes_dict['X']) > 0 else '')
        u = _wrap_quotes(f"U : {random.choice(self.nodes_dict['U'])}" if len(
            self.nodes_dict['U']) > 0 else '')
        w = _wrap_quotes(f"W : {random.choice(self.nodes_dict['W'])}" if len(
            self.nodes_dict['W']) > 0 else '')
        y = _wrap_quotes(f"Y : {random.choice(self.nodes_dict['Y'])}" if len(
            self.nodes_dict['Y']) > 0 else '')
        l = _wrap_quotes(f"L : {random.choice(self.nodes_dict['dec'])}" if len(
            self.nodes_dict['dec']) > 0 else '')
        m = _wrap_quotes(f"M : {random.choice(self.nodes_dict['acc'])}" if len(
            self.nodes_dict['acc']) > 0 else '')
    ...
    @staticmethod
    def from_str(igraph_str: str, graph_id: str = "", para_id: str = "",
