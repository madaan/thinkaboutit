import logging
import pytorch_lightning as pl
import torch
from torch.utils.data import DataLoader
from torch.utils.data import Dataset
import pandas as pd
from transformers import AutoTokenizer
from tqdm import tqdm
from collections import defaultdict
from src.gnn_qa.influence_graph import InfluenceGraph

label_dict = {"less": 0, "attenuator": 0, "more": 1, "intensifier": 1, "no_effect": 2}
rev_label_dict = defaultdict(list)

for k, v in label_dict.items():
    rev_label_dict[v].append(k)

rev_label_dict = {k: "/".join(v) for k, v in rev_label_dict.items()}


class GraphQaDataModule(pl.LightningDataModule):
    def __init__(
        self,
        basedir: str,
        tokenizer_name: str,
        batch_size: int,
        use_graphs: bool,
        graphs_file_name: str,
        format: str,
        merge_nodes_op: str,
        num_workers: int = 64,
        **kwargs,
    ):
        super().__init__()
        self.basedir = basedir
        self.graphs_file_name = graphs_file_name
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.merge_nodes_op = merge_nodes_op
        self.use_graphs = use_graphs
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, do_lower_case=True)
        self.format = format

    def train_dataloader(self):
        dataset = GraphQADataset(
            tokenizer=self.tokenizer,
            format=self.format,
            qa_pth=f"{self.basedir}/qa-train.jsonl",
            graph_pth=f"{self.basedir}/{self.graphs_file_name}",
            merge_nodes_op=self.merge_nodes_op,
            use_graphs=self.use_graphs,
        )
        return DataLoader(
            dataset=dataset,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=self.num_workers,
            collate_fn=GraphQADataset.collate_pad,
        )

    def val_dataloader(self):
        dataset = GraphQADataset(
            tokenizer=self.tokenizer,
            format=self.format,
            qa_pth=f"{self.basedir}/qa-dev.jsonl",
            graph_pth=f"{self.basedir}/{self.graphs_file_name}",
            merge_nodes_op=self.merge_nodes_op,
            use_graphs=self.use_graphs,
        )
        return DataLoader(
            dataset=dataset,
            batch_size=self.batch_size,
            shuffle=False,
            num_workers=self.num_workers,
            collate_fn=GraphQADataset.collate_pad,
        )

    def test_dataloader(self):
        dataset = GraphQADataset(
            tokenizer=self.tokenizer,
            format=self.format,
            qa_pth=f"{self.basedir}/qa-test.jsonl",
            graph_pth=f"{self.basedir}/{self.graphs_file_name}",
            merge_nodes_op=self.merge_nodes_op,
            use_graphs=self.use_graphs,
        )
        return DataLoader(
            dataset=dataset,
            batch_size=self.batch_size,
            shuffle=False,
            num_workers=self.num_workers,
            collate_fn=GraphQADataset.collate_pad,
        )


class GraphQADataset(Dataset):
    def __init__(
        self, tokenizer, qa_pth: str, graph_pth: str, format: str, use_graphs: bool, merge_nodes_op: str
    ) -> None:
        super().__init__()
        self.qa_pth = qa_pth
        self.graph_pth = graph_pth
        self.use_graphs = use_graphs
        self.tokenizer = tokenizer
        self.format = format
        self.merge_nodes_op = merge_nodes_op
        if use_graphs:
            self.read_graphs()
        else:
            logging.info("Graphs are NOT used for this run!")
        self.read_qa()

    def read_graphs(self):
        influence_graphs = pd.read_json(self.graph_pth, orient="records", lines=True).to_dict(orient="records")
        self.graphs = {}
        for graph_dict in tqdm(
            influence_graphs, desc=f"Reading graphs from {self.graph_pth}", total=len(influence_graphs)
        ):
            self.graphs[str(graph_dict["graph_id"])] = InfluenceGraphNNData.make_str_from_dict(
                graph_dict, self.tokenizer, merge_nodes_op=self.merge_nodes_op
            )
        
    def read_qa(self):
        logging.info("Reading data from {}".format(self.qa_pth))
        data = pd.read_json(self.qa_pth, orient="records", lines=True)
        self.questions, self.answer_labels, self.graph_ids, self.paragraphs = [], [], [], []
        logging.info(f"Reading QA file from {self.qa_pth}")
        self.PHUs = []
        for i, row in tqdm(data.iterrows(), total=len(data), desc="Reading QA examples"):
            self.answer_labels.append(row["question"]["answer_label"].strip())
            self.graph_ids.append(row["metadata"]["graph_id"])
            graph = self.graphs[row["metadata"]["graph_id"]] if self.use_graphs else ""
            para = " ".join([p.strip() for p in row["question"]["para_steps"] if len(p) > 0])
            question = row["question"]["stem"].strip()
            self.questions.append(question)
            self.paragraphs.append(graph + " " + para)
            if self.format == "phu":
                self.PHUs.append(f"{row['premise']} {row['hypo']}</s></s>{row['update']}")
            elif self.format == "puh":
                self.PHUs.append(f"{row['premise']}</s></s>{row['update']}</s></s>{row['hypo']}")
            elif self.format == "puhy":
                self.PHUs.append(f"{row['premise']}</s></s>{row['update']}</s></s>{row['hypo']}</s></s>{graph}")
            elif self.format == "phuy":
                self.PHUs.append(f"{row['premise']} {row['hypo']}</s></s>{row['update']}</s></s>{graph}")
            elif self.format == "phyu":
                self.PHUs.append(f"{row['premise']} {row['hypo']} {graph}</s></s>{row['update']}")
            elif self.format == "puyh":
                self.PHUs.append(f"{row['premise']} {row['update']} {graph}</s></s>{row['hypo']}")
            elif self.format == "natural":
                self.PHUs.append(f"Given {row['premise']} , {row['update']} affects {row['hypo']} via {graph}</s></s>")

        # encoded_input = self.tokenizer(self.questions, self.paragraphs)
        encoded_input = self.tokenizer(self.PHUs)
        self.input_ids = encoded_input["input_ids"]
        if "token_type_ids" in encoded_input:
            self.token_type_ids = encoded_input["token_type_ids"]
        else:
            # only BERT uses it anyways, so just set it to 0
            self.token_type_ids = [[0] * len(s) for s in encoded_input["input_ids"]]

    def __len__(self) -> int:
        return len(self.questions)

    def __getitem__(self, i):
        # We’ll pad at the batch level.
        return (
            self.input_ids[i],
            self.token_type_ids[i],
            self.graphs[self.graph_ids[i]] if self.use_graphs else "",
            self.answer_labels[i],
        )

    @staticmethod
    def collate_pad(batch, cutoff: int = 500):
        max_token_len = 0
        num_elems = len(batch)
        for i in range(num_elems):
            tokens, _, _, _ = batch[i]
            max_token_len = max(max_token_len, len(tokens))
        max_token_len = min(cutoff, max_token_len)
        tokens = torch.zeros(num_elems, max_token_len).long()
        tokens_mask = torch.zeros(num_elems, max_token_len).long()
        token_type_ids = torch.zeros(num_elems, max_token_len).long()
        labels = torch.zeros(num_elems).long()
        graphs = []
        for i in range(num_elems):
            toks, type_ids, graph, label = batch[i]
            length = len(toks)
            tokens[i, :length] = torch.LongTensor(toks)
            token_type_ids[i, :length] = torch.LongTensor(type_ids)
            tokens_mask[i, :length] = 1
            graphs.append(graph)
            labels[i] = label_dict[label]
        return [tokens, token_type_ids, tokens_mask, graphs, labels]


class InfluenceGraphNNData:
    """
    V       Z
    |     /
    -   +
    | /
    X       U
    | \     |
    -   +   -
    |     \ |
    W       Y
    | \   / |
    -   +   -
    | /   \ |
    L       M
    """

    node_index = {"V": 0, "Z": 1, "X": 2, "U": 3, "W": 4, "Y": 5, "dec": 6, "acc": 7}
    index_node = {v: k for k, v in node_index.items()}
    edge_index = [[0, 1, 2, 2, 3, 4, 4, 5, 5], [2, 2, 4, 5, 5, 6, 7, 6, 7]]
    EDGE_TYPE_HELPS, EDGE_TYPE_HURTS = 0, 1

    y_affects = {"more": "helps", "less": "hurts"}

    y_affects_inv = {"more": "hurts", "less": "helps"}

    def __init__(self, data) -> None:
        super().__init__()
        self.data = data

    @staticmethod
    def make_str_from_dict(graph_dict: dict, tokenizer, merge_nodes_op: str, max_length=200):

        graph = InfluenceGraph(graph_dict)
        return graph.nodes_to_string(merge_nodes_op=merge_nodes_op)


if __name__ == "__main__":
    import sys

    dm = GraphQaDataModule(basedir=sys.argv[1], model_name=sys.argv[2], batch_size=32)
    for (tokens, tokens_mask, graphs, labels) in dm.train_dataloader():
        print(torch.tensor(graphs[0].tokens).shape)
