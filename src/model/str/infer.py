...
def load_model(ckpt, graphs_file_name):
    ...
    dm = GraphQaDataModule(
        ...
        format=model.hparams.format,
        merge_nodes_op=model.hparams.merge_nodes_op,
        use_graphs=model.hparams.use_graphs if "use_graphs" in model.hparams else True,
    )
...
def eval(model, dataloader, influence_graphs, extract_paths: bool = False, paths_output_loc: str = None):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=len(dataloader)):
            ...
            attention, logits, acc = model(
                [question_tokens.cuda(), question_type_ids.cuda(), question_masks.cuda(), graphs, labels.cuda()]
            )
